...
public class BaiduMapUtil {
	...
	public static List<BaiduPlace> searchPlace(String query, String lng, String lat) throws Exception {
		// 拼装请求地址
		String requestUrl = "http://api.map.baidu.com/place/v2/search?&query=QUERY&location=LAT,LNG&radius=3000&output=xml&scope=2&page_size=10&page_num=0&ak=AKcoyRWeP64QlvZCFKbs3zMb";
	...
	public static List<Article> makeArticleList(List<BaiduPlace> placeList, String bd09Lng, String bd09Lat) {
		// 项目的根路径
		String basePath = "http://1.weichattestpage.sinaapp.com/";
		List<Article> list = new ArrayList<Article>();
		BaiduPlace place = null;
		for (int i = 0; i < placeList.size(); i++) {
			place = placeList.get(i);
			//以文章的形式返回给用户 wei 以这种方式可以扩展到其他功能，如返回music、video形式的
			Article article = new Article();
			article.setTitle(place.getName() + "\n距离您约" + place.getDistance() + "米");
